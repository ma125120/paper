  ...
  <div class="student-info">
    ...
    <b-table :fields="options" :items="topics" hover>
      ...
      <template slot="options" slot-scope="data">
        <b-button @click="changeStatus(data.item.id)" 
                  variant="primary"
                  :disabled="data.item.status==1">通过审核</b-button>
      </template>
...
export default {
  ...
  methods:{
    getInfo(page=0,size=10) {
      this.$http.get(`/admin/topic/getTopicByPage?page=${page}&size=${size}`)
      .then(({ data })=>{
        this.topics=data.data;
      }).catch(err=>{console.log(err)});
    },
    changeStatus(id) {
      this.$http.get(`/admin/topic/changeStatus?id=${id}`)
      .then(({ data })=>{
        this.msg=data.msg;
        this.$refs.tip.show();
      }).catch(err=>{console.log(err)});
